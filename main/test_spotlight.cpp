...
namespace gpr5300
{
	...
	private:
		...
		float tt_ = 0.0f;
		std::string_view cubeVert = "data/shaders/hello_triangle/spotlight.vert";
		std::string_view cubeFrag = "data/shaders/hello_triangle/spotlight.frag";
	...
	void Test::Update(float dt)
	{
		...
		pipelinecube_.SetVec3("light.position", camera_.cameraPos);
		pipelinecube_.SetVec3("light.direction", camera_.cameraFront);
		pipelinecube_.SetFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
		pipelinecube_.SetFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));
		...
		pipelinecube_.SetFloat("light.constant", 1.0f);
		pipelinecube_.SetFloat("light.linear", 0.09f);
		pipelinecube_.SetFloat("light.quadratic", 0.032f);
		CheckError(__FILE__, __LINE__);
